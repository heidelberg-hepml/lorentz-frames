...
def gram_schmidt_lorentz(
    ...
    for index in range(vec.shape[0]):
        ...
        while error == True:
            sign = (
                leinsum("vds,vds->vs", vec[:index], vec[:index], dim=-2)
                .sign()
                .unsqueeze(-2)
            )
            weights = leinsum("vds,ds->vs", sign * vec[:index], vec[index], dim=-2)
            normBefore = (
                leinsum("ds,ds->s", vec[index], vec[index], dim=-2).abs().sqrt()
            )
            vec[index] -= torch.sum(
                torch.einsum("vs,vds->vds", weights, vec[:index]), axis=0
            )
            ...
            if zeroNorm.sum().item() != 0:  # linearly alligned elements / zero norm
                vec[index, :, zeroNorm] = torch.rand(
                    vec[index, :, zeroNorm].shape, device=device
                )
                errorCounter += 1
            else:
    ...
    if vectors.shape[:-1] == (3, 4):
        ...
        vec = torch.cat(
            (vec, torch.zeros(1, vectors.shape[1], vectors.shape[-1])), dim=0
        )

        vec[-1, 3] = ((z[0] - alpha2 * x[0]) - beta * (y[0] - alpha1 * x[0])) / (
            (z[3] - alpha2 * x[3]) - beta * (y[3] - alpha1 * x[3])
        )
        vec[-1, 2] = ((y[0] - alpha1 * x[0]) - (y[3] - alpha1 * x[3]) * vec[-1, 3]) / (
            y[2] - alpha1 * x[2]
        )
