...
def pairwise_lv_fts(xi, xj, num_outputs=4, eps=1e-8, for_onnx=False):
    ...
    delta = delta_r2(rapi, phii, rapj, phij).clamp(min=eps * eps).sqrt()  # clamp BEFORE sqrt: sqrt(0) backward is 0/0=NaN, silently poisoning framesnet grads on bit-identical pairs; forward unchanged (log clamps at eps anyway)
